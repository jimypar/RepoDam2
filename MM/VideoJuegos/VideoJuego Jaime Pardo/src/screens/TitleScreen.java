...
public class TitleScreen extends BScreen {
	...
	public TitleScreen(Demo game) {
		super(game);
		if (SoundManager.currentMusicName != "Menu/MUS_InkwellIsleOne.ogg") {
			SoundManager.playMusic("Menu/MUS_InkwellIsleOne.ogg");
		}
		...
		boton.addListener((Event e) -> {
			if (!(e instanceof InputEvent) || !((InputEvent) e).getType().equals(Type.enter))
				return false;
		...
		boton.addListener((Event e) -> {
			if (!(e instanceof InputEvent) || !((InputEvent) e).getType().equals(Type.exit))
				return false;
