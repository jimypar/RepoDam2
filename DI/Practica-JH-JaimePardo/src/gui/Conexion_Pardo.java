...
public class Conexion_Pardo {
    ...
    void insertarAutor(String dni, String nombre, String apellidos, String direccion, Integer cp, String ciudad, Integer telefono) {
        String sentenciaSql = "INSERT INTO cliente (dni, nombre, apellidos, direccion, cp, ciudad, telefono)" +
                "VALUES (?,?,?,?,?,?,?)";
    ...
    ResultSet consultarAutor() throws SQLException {
        String sentenciaSql = "SELECT concat(id_cliente) AS 'ID', concat(dni) AS 'DNI', concat(nombre) AS 'Nombre', concat(apellidos) AS 'Apellidos', " +
                "concat(direccion) AS 'Direccion',concat(ciudad) AS 'Codigo Postal',concat(cp)  AS 'Ciudad', concat(telefono) AS 'Telefono' FROM cliente";
        PreparedStatement sentencia = null;
